...
/*
·    ____           __                   __   _____      __  
·   / __ \_________/ /__  ________  ____/ /  / ___/___  / /_ 
·  / / / / ___/ __  / _ \/ ___/ _ \/ __  /   \__ \/ _ \/ __/ 
· / /_/ / /  / /_/ /  __/ /  /  __/ /_/ /   ___/ /  __/ /_   
· \____/_/   \__,_/\___/_/   \___/\__,_/   /____/\___/\__/   
·                                                            
·        ______                 __  _                        
...
SELECT
    Adoption_Date
  , SUM(Adoption_Fee) AS Total_Fee
  , STRING_AGG (CONCAT (AN.Name, ' the ', AN.Breed, ' ', AN.Species), ', ') WITHIN GROUP (
        ORDER BY
            AN.Species
          , AN.Breed
          , AN.Name
    ) AS Using_CONCAT
  , STRING_AGG (AN.Name + ' the ' + AN.Breed + ' ' + AN.Species, ', ') WITHIN GROUP (
        ORDER BY
            AN.Species
          , AN.Breed
          , AN.Name
...
FROM
    Adoptions AS AD
    INNER JOIN Animals AS AN ON AN.Name = AD.Name
...
-- Commented: PostgreSQL
WITH
    Vaccination_Ranking AS (
        SELECT
            Name
          , Species
...
SELECT
  ...
  , ROUND(AVG(Number_Of_Vaccinations), 2) AS AVG_Vaccinations
    --CAST( ... AS DECIMAL(9, 2))
    --, DENSE_RANK(5) WITHIN GROUP ( -- X=5
    --      ORDER BY
    --          Number_Of_Vaccinations DESC
    --  ) AS How_Would_X_Rank
    --, PERCENT_RANK(5) WITHIN GROUP (
    --      ORDER BY
    --          Number_Of_Vaccinations DESC
    --  ) AS How_Would_X_Rank_Percent_Wise
    --, PERCENTILE_CONT(0.333) WITHIN GROUP (
    --      ORDER BY
    --          Number_Of_Vaccinations DESC
    --  ) AS Inverse_Continous
    --, PERCENTILE_DISC(0.333) WITHIN GROUP (
    --      ORDER BY
    --          Number_Of_Vaccinations DESC
    --  ) AS Inverse_Discrete
FROM
